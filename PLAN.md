...
| **Landing gear that actually moves** *(not originally a phase)* | `glb2hulls` no longer bakes the gear rig into the hull: a node ending in `Hinge` **inside a `gear*` leg** comes out as its own small mesh carrying the pivot and axis it turns about, with a `stow` angle MEASURED off the model. 36 of 52 hulls are rigged (6–10 parts each); the orbital-only classes — capital, liner, navy, freighter, escape pod — have no gear and correctly report `NO GEAR`. The renderer needed no new concept: a part is a mesh and a frame, the same shape `drawPortDoors` uses for a sliding leaf. `ship.gearTravel` eases 0→1 over 2.4 s of sim time while `ship.gear` stays the switch every rule already reads. **This also fixed a standing bug nobody had named**: the models were saved gear-down, so every ship in the game had three legs welded to its belly, permanently extended, in flight and in combat |
